        ...
        {loading && !selectedBidDetails && bids.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 space-y-3">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-slate-600">
              Initializing SATYAM Platform & Simulated Government Registries...
            </p>
          </div>
        ) : (
          <>
            ...
            {(activeTab === 'dossier' || activeTab === 'bids') && !selectedBidDetails && (
              <div className="bg-white p-8 rounded-xl border border-slate-200 text-center space-y-3 shadow-2xs">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold transition shadow-xs cursor-pointer"
                >
            ...
            {activeTab === 'tenders' && (
              <TendersListView
                ...
                onOpenNewTender={() => setIsNewTenderOpen(true)}
                onTenderUpdated={loadData}
              />
            )}

            {activeTab === 'evaluations' && (
              <EvaluationsOverviewView
                bids={bids}
                onSelectBid={handleSelectBid}
                onRefreshAll={loadData}
              />
            ...
            {activeTab === 'reports' && (
              <ReportsView
                ...
                selectedBidId={selectedBidDetails?.id}
                currentUser={currentUser}
                onSelectBid={handleSelectBid}
                onOpenPrintModal={(bidToPrint) => {
                  setSelectedBidDetails(bidToPrint);
            ...
            {!['dashboard', 'dossier', 'bids', 'tenders', 'evaluations', 'reports', 'apis', 'audit'].includes(activeTab) && (
              <NotFoundView onBackToDashboard={() => setActiveTab('dashboard')} />
            )}
          </>
        )}
      </main>
